...
public class PostgresRecordReader implements RecordReader, DrillPostgresConstants {
  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(PostgresRecordReader.class);
  ...
  public PostgresRecordReader(Configuration conf, PostgresSubScan.PostgresSubScanSpec subScanSpec,
    ...
    try {
      ...
      if (rowKeyOnly) {
         ...
         */
        scan.setFilter(
            PostgresUtils.andFilterAtIndex(scan.getFilter(), PostgresUtils.LAST_FILTER, new FirstKeyOnlyFilter())
        );
      }
      scan.setCaching(TARGET_RECORD_COUNT);

      logger.debug("Opening scanner for Postgres table '{}', Zookeeper quorum '{}', port '{}', znode '{}'.",
      ...
      try {
        ...
        logger.warn("Failure while closing Postgres table: " + subScanSpec.getTableName(), e);
      }
